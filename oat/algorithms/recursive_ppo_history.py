...
class PPOActor(RewardActor):
    ...
    def generate_reasoning(self, x_text, y_text, z_text, y_history, z_history):
        sys_prompt_reasoning = f"""
        You are a reasoning model that is given an input prompt, a current output, and a current reasoning block. 
        Your goal is to improve the reasoning block.
        Output the improved reasoning block.
        History of outputs: {''.join(y_history)}
        History of reasoning blocks: {''.join(z_history)}

        Input prompt: {x_text}
        Current output: {y_text}
        Output reasoning block: {z_text}
        """ 

        prompts = [sys_prompt_reasoning]
        outputs = self.generate(prompts, self.sampling_params)

        return outputs[0], y_history, z_history + [outputs[0].outputs[0].text]

    def generate_refine(self, y_text, z_text, y_history, z_history):
        sys_prompt_refine = f"""
        You are a reasoning model that is given a current output, and a current reasoning block.  
        Your goal is to improve the output based on the reasoning block, which you have generated.
        Output the improved output.
        History of outputs: {''.join(y_history)}
        History of reasoning blocks: {''.join(z_history)}
        ...
        prompts = [sys_prompt_refine]
        outputs = self.generate(prompts, self.sampling_params)

        return outputs[0], y_history + [outputs[0].outputs[0].text], z_history
    ...
    def step(
        ...
        def latent_generative_recursion(x_text, y_text, z_text, n=6):
            if type(y_text) == str:

                for i in range(n):
                    z_text = self.generate_reasoning(x_text, y_text, z_text)
                y_text = self.generate_refine(y_text, z_text)
                return y_text, z_text
            else:
                for i in range(n):
                    z_text = self.generate_reasoning(x_text, y_text.outputs[0].text, z_text.outputs[0].text)
                y_text = self.generate_refine(y_text.outputs[0].text, z_text.outputs[0].text)
                return y_text, z_text


        def deep_recursion(x_text, y_text, z_text, n=6, T=3):

            with torch.no_grad():
                for j in range(T):
                    if type(y_text) == str:
                        y_text, z_text = latent_generative_recursion(x_text, y_text, z_text, n)
                    else:
                        y_text, z_text = latent_generative_recursion(x_text, y_text.outputs[0].text, z_text.outputs[0].text, n)
            return y_text, z_text
        ...
        for input in formatted_prompts: 
            ...
            x_text = input 
            y_text = "This is where I will put the final output."
            z_text = "This is where I will put my reasoning."

            y_text, z_text = deep_recursion(x_text, y_text, z_text)

            outputs.append(y_text)
        ...
        for i in range(len(outputs)):
            ...
            response_ids.append([])
            # let's keep n at 1 for now since i am low on brain cells and this is a simple experiment
            n = 1
