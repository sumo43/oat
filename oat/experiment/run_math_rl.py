...
class ZeroMathArgs(PPOArgs):
    ...
    # Ablations.
    critic_type: Literal["ppo", "grpo", "drgrpo", "dapo"] = "drgrpo"
...
class ZeroMathLearner(PPOLearner):
    def _init(self, args: ZeroMathArgs, actors: List[ActorBase]) -> None:
        ...
        if args.critic_type == "dapo":
            # DAPO-like loss is equivalent to summed loss with pre-normalized advantages;
            # See `compute_monte_carlo_advantages` below for the pre-normalization.
            self.masked_aggregator = functools.partial(
                masked_sum, constant_normalizer=1
            )
    ...
    def learning_step(self, trajectory):
        ...
        with torch.no_grad():
            for i in range(0, len(input_ids), args.train_batch_size_per_device):
                ...
                # NOTE(liuzc): The order of temperature and min_p need to follow that of sampler.
                # For now: https://github.com/vllm-project/vllm/blob/main/vllm/v1/sample/sampler.py#L108
    ...
    def compute_monte_carlo_advantages(self, rewards, response_masks):
        ...
        if (self.args.critic_type in ["grpo", "dapo"]) and (
            not self.args.remove_std_bias
        ):
        ...
        if self.args.critic_type == "dapo":
            # Pre-normalize using summed response length within the group.
            pass
        return advantages
