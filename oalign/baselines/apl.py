...
from oalign import actor
from oalign.learners.dap import DAPLearner
from oalign.model import LLM
from oalign.types import Metric, PreferenceData
from oalign.utils.data import zero_pad_sequences
from oalign.utils.ipc import DataID, PlasmaShmClient
...
class APLActor(actor.Actor):
    ...
    def generate_and_entropy_filter(self, prompts: List[str]) -> DataID:
        ...
        logging.debug(
            "generated", len(outputs), len(outputs[0].outputs), self.sampling_params
        )
        ...
        if not self.args.apl_pref_certainty_only:
            ...
            logging.debug("filtered", len(outputs), len(outputs[0].outputs))
    ...
    def run(self):
        ...
        for p_ep in range(self.args.num_prompt_epoch):
            ...
            for processed_prompts, raw_prompts, refs in self.prompts_dataloader:
                ...
                logging.debug(
                    f"Reward margin filtering: {output_info1} -> {output_info2}"
                )
